...
class ScrapifyAdminApiClient {
  ...
  async sendEmailOtpTest(email: string) {
    return this.request<any>("/admin/otp-settings/test-email", {
      method: "POST",
  ...
  async getKybDetails(id: number | string) {
    return this.request<any>(`/admin/kyb/${id}`);
  }
  async approveKyb(id: number | string, reason: string) {
    return this.request<any>(`/admin/kyb/${id}/approve`, {
      method: "POST",
      body: JSON.stringify({ reason }),
    });
  }
  async rejectKyb(id: number | string, reason: string) {
    return this.request<any>(`/admin/kyb/${id}/reject`, {
      method: "POST",
      body: JSON.stringify({ reason }),
    });
  }
  async requestKybReverification(id: number | string, reason: string) {
    return this.request<any>(`/admin/kyb/${id}/request-reverification`, {
      method: "POST",
      body: JSON.stringify({ reason }),
    });
  }
  ...
  async resolveDispute(
    ...
    status: "resolved" | "closed" | "appealed",
  ) {
  ...
  async resolveRiskFlag(
    code: string,
    status: "resolved" | "false_positive" | "restricted",
    notes?: string,
  ) {
  ...
  async startRefund(
    ledgerId: number,
    data: { refund_method: string; amount?: number; notes?: string } = { refund_method: "MANUAL" },
  ) {
    return this.request<any>(`/settlement/ledger/${ledgerId}/refund/start`, {
      method: "POST",
      body: JSON.stringify(data),
    });
  }
  ...
  async completeRefund(ledgerId: number, reference: string) {
    return this.request<any>(`/settlement/ledger/${ledgerId}/refund/complete`, {
      method: "POST",
  ...
  async applyLossAdjustment(ledgerId: number, amount: number, reason: string, policy: string) {
    return this.request<any>(`/settlement/ledger/${ledgerId}/loss-adjustment`, {
      method: "POST",
      body: JSON.stringify({ amount, reason, policy }),
    });
  }
  ...
  async defaultWinner(awardId: number, reason: string, forfeit_emd = false) {
    return this.request<any>(`/awards/${awardId}/default`, {
      method: "POST",
      body: JSON.stringify({ reason, forfeit_emd }),
    });
  }
  ...
  async releaseFallbackEmd(auctionCode: string) {
    return this.request<any>(`/auctions/${auctionCode}/settlement/release-fallback`, {
      method: "POST",
    });
  }
  ...
  async declineAward(awardId: number, reason: string) {
    return this.request<any>(`/awards/${awardId}/decline`, {
      method: "POST",
      body: JSON.stringify({ reason }),
    });
  }
  ...
  async updateAuctionConfiguration(code: string, data: Record<string, unknown>) {
    return this.request<any>(`/auctions/${encodeURIComponent(code)}/configuration`, {
      method: "PATCH",
      body: JSON.stringify(data),
    });
  }
